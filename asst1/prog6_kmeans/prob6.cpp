...
void kMeansThread(double *data, double *clusterCentroids, int *clusterAssignments,
  ...
  double compute_assignment_time = 0;
  double compute_centroid_time = 0;
  double compute_cost_time = 0;
  ...
  while (!stoppingConditionMet(prevCost, currCost, epsilon, K)) {
    ...
    WorkerArgs thread_args[MAX_THREADS];

    //computeAssignments with threads
    ...
    for (int i=1; i<MAX_THREADS; i++) {
        workers[i] = std::thread(computeAssignments, &thread_args[i]);
    }
    computeAssignments(&thread_args[0]);
    compute_assignment_time += CycleTimer::currentSeconds() - startTime;
    for (int i=1; i<MAX_THREADS; i++) {
        workers[i].join();
    }
    ...
    compute_centroid_time += (CycleTimer::currentSeconds() - startTime);
    startTime = CycleTimer::currentSeconds();
    computeCost(&args);
    compute_cost_time += (CycleTimer::currentSeconds() - startTime);
    iter++;
  }

  free(currCost);
  free(prevCost);
  printf("compute assignment time: %.3f\ncompute centroid time: %.3f\ncompute cost time: %.3f\n", compute_assignment_time,compute_centroid_time,compute_cost_time);
}
