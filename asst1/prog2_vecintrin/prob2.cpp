...
void clampedExpVector(float* values, int* exponents, float* output, int N) {
  ...
  __cs149_vec_float compare = _cs149_vset_float(9.999999f);
  __cs149_vec_float one = _cs149_vset_float(1.f);
  __cs149_mask y_mask, maskAll, y_notzero, cnt_mask;

  int vector_packs = (int) N/VECTOR_WIDTH;

  for (int i=0; i<N; i+=VECTOR_WIDTH) {
    // All ones
    if(i==(vector_packs*VECTOR_WIDTH) && (N%VECTOR_WIDTH) != 0) {
    ...
    // if (y==0)
    _cs149_veq_int(y_mask, y, zero, maskAll);

    // output[i] = 1.f;
    _cs149_vstore_float(output+i, one, y_mask);

    // else
    y_notzero = _cs149_mask_not(y_mask);
    y_notzero = _cs149_mask_and(y_notzero, maskAll);

    // float result = x;
    _cs149_vmove_float(result, x, y_notzero);

    // int count = y - 1;
    _cs149_vsub_int(count, y, ione, y_notzero);

    // count > 0
    _cs149_vgt_int(cnt_mask, count, zero, y_notzero);

    // Execute until all the lanes have computed exponents
    while(_cs149_cntbits(cnt_mask) > 0) {

        _cs149_vmult_float(result, result, x, cnt_mask);

        // count = count - 1;
        _cs149_vsub_int(count, count, ione, cnt_mask);

        // Updating the cnt_mask
        _cs149_vgt_int(cnt_mask, count, zero, cnt_mask);
    }

    // if (result > 9.999999f)
    _cs149_vgt_float(cnt_mask, result, compare, y_notzero);

    // result = 9.999999f;
    _cs149_vmove_float(result, compare, cnt_mask);

    // Write results back to memory
    _cs149_vstore_float(output+i, result, y_notzero);
